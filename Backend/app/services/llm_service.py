...
class LLMService:
    ...
    def detect_clarification_options(self, query: str, context_items: List[str]):
        """Focused disambiguation check for BROAD/ambiguous questions.

        Decides whether `query` is a broad term that maps to 2+ genuinely DISTINCT subtypes
        present in the retrieved `context_items`. This is a small single-purpose call — far more
        reliable than asking the big answer prompt to withhold an answer and clarify.

        Returns (topic, options): `topic` is the short ambiguous term (e.g. "validation") and
        `options` is a list of 2-6 distinct subtype labels drawn ONLY from the context. Returns
        ("", []) when the question is specific or the material is really one topic. Safe: on any
        error returns ("", []) so the normal answer flow proceeds.
        """
        if not context_items:
            return "", []

        # The "ambiguous or not?" call is a tricky same-vs-distinct judgment — mini over-splits,
        # so this ONE decision uses the stronger judge model (config-driven, defaults to gpt-5.5).
        # The actual answer writing still uses self.model (mini), so normal answers stay cheap.
        model = settings.OPENAI_CLARIFY_MODEL
        material = "\n".join(f"- {c}" for c in context_items[:20])
        system_prompt = (
            "You decide whether a user's question is too BROAD to answer directly because the "
            "retrieved material covers SEVERAL genuinely DISTINCT subtypes of the asked term, so "
            "the user should first pick which one they mean.\n"
            "Return subtype labels ONLY if there are 2 or more clearly different subtypes in the "
            "material. If the question already names a specific subtype, or the material is really "
            "one topic, return an empty list.\n"
            "Each label must be SHORT (2-5 words), grounded in the material, and mutually distinct.\n"
            'Respond with ONLY JSON: {"topic": "<the broad term, e.g. validation>", '
            '"options": ["<label>", ...]}. If not ambiguous, use {"topic": "", "options": []}.'
        )
        ...
        try:
            ...
            if len(options) >= 2:
                return topic, options[:6]
            return "", []
        except Exception as e:
            logger.error(f"Clarification detection failed ({e}); proceeding with normal answer.")
            return "", []
